...
if ( ! class_exists( 'Editorial_Settings' ) ) :
	...
	?>
			<div id="mt-theme-message" class="updated notice editorial-message">
				...
				<div class="submit">
					<button class="mt-get-started button button-primary button-hero" data-done="<?php esc_html_e( 'Done!', 'editorial' ); ?>" data-process="<?php esc_html_e( 'Processing', 'editorial' ); ?>" data-redirect="<?php echo esc_url( wp_nonce_url( add_query_arg( 'editorial-hide-notice', 'welcome', admin_url( 'themes.php' ).'?page=editorial-settings&tab=demos' ) , 'editorial_hide_notices_nonce', '_editorial_notice_nonce' ) ); ?>">
	...
	<?php
		...
		public function demos_screen() {
			...
			if ( empty( $demodata ) ) {
				$editorial_library = new Editorial_Demo_Library();
				$demodata = $editorial_library->retrieve_demo_by_activatetheme( $activated_theme );
				if ( $demodata ) {
						...
						<div class="mt-theme-demos rendered">
							...
							<?php
								if ( isset( $demodata ) && empty( $demodata ) ) {
									esc_html_e( 'No demos are configured for this theme, please contact the theme author', 'editorial' );
									return;
								} else {
							?>
